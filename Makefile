...
ARTIFACT_DATA_DEPLOY_PATH = ${ARTIFACT_DIR}_run/app.data
ARTIFACT_HTML_DEPLOY_PATH = ${ARTIFACT_DIR}_run/index.html
...
run:
	python3 -m http.server -d build/$(ARTIFACT)_run
...
deploy:
	...
	cp ${ARTIFACT_DATA} ${ARTIFACT_DATA_DEPLOY_PATH}
	cp $(ARTIFACT_HTML) ${ARTIFACT_HTML_DEPLOY_PATH}
